    ...
    def _calc_face_plane_equations(self) -> np.ndarray:
        ...
        for i in range(4):
            normal_vector = np.cross(
                self.vertices[:, i] - next_vertices[:, i], self._surface_normal
            )
...
class SurfaceCell:
    def __init__(
        ...
        molecule_width = (
            molecule_width if not molecule_width is None else molecular_width(self.original_cell)
        )
        self._unit_height = self.original_cell.lattice.matrix[-1] @ self._surface_normal
        self._c_scale = math.ceil(molecule_width / self._unit_height)
        self.structure, self.height = self._derive_structure()
        if debug:
            self.structure.to(os.path.join(os.getcwd(), "buffered_structure.cif"))
    ...
    def _derive_structure(self) -> Structure:
        ...
        raw_supercell = self.original_cell.make_supercell(
        ...
        surface_prism = SurfacePrism.from_matrix(
            self.original_cell.lattice.matrix, self._surface_normal
        )
        ...
        for molecule_graph in get_molecule_graphs(supercell_slug):
        ...
        matrix = self.original_cell.lattice.matrix.copy()
        ...
        supercell = structure.make_supercell([3, 3, 1], to_unit_cell=True, in_place=False)
        buffered_prism = SurfacePrism.from_matrix(structure.lattice.matrix, self._surface_normal)
        buffered_structure = buffered_prism.buffer_mask_supercell(supercell, in_place=False)

        return buffered_structure, max_head

    def bounds(self) -> np.ndarray:
        a, bx, by = (
            self.original_cell.lattice.matrix[0, 0],
            self.original_cell.lattice.matrix[1, 0],
            self.original_cell.lattice.matrix[1, 1],
        )
        x_bounds = [bx, a] if bx < 0 else [0, bx + a]
        y_bounds = [by, 0] if by < 0 else [0, by]
        z_bounds = [0, self.height]
...
class SurfaceVoxels:
    def __init__(
        ...
        stop_timer("SurfaceVoxels._voxel_surface()")
        start_timer("SurfaceVoxels._roughnesses()")
        self._roughnesses = self._roughnesses()
        self._average_roughness = self._average_roughness()
...
class Termination:
    def __init__(
        self,
        rotated_dummy_structure: Structure,
        shift: float,
        ...
        self._structures = {
            "rotated": {"dummy": rotated_dummy_structure, "molecular": rotated_molecular_structure},
            "unrotated": {
    ...
    def rotated_dummy_structure(self) -> Structure:
        return self._structures["rotated"]["dummy"]
    ...
    def unrotated_dummy_structure(
        ...
        if self._structures["unrotated"]["dummy"]:
            return self._structures["unrotated"]["dummy"]
    ...
    def rotated_molecular_structure(self, overwrite: bool = False) -> Structure:
        if self._structures["rotated"]["molecular"]:
            return self._structures["rotated"]["molecular"]

        molecular = add_molecules(structure=self._structures["rotated"]["dummy"])
        if overwrite:
    ...
    def unrotated_molecular_structure(
        ...
        if self._structures["unrotated"]["molecular"]:
            return self._structures["unrotated"]["molecular"]

        if self._structures["unrotated"]["dummy"] is None:
            dummy = (
                self._unrotate(
                    structure=self._structures["rotated"]["dummy"],
                    unrotation_transformations=unrotation_transformations,
            ...
            if overwrite:
                self._structures["unrotated"]["dummy"] = dummy
        else:
            dummy = self._structures["unrotated"]["dummy"]

        molecular = add_molecules(structure=dummy)
        if overwrite:
            self._structures["unrotated"]["molecular"] = molecular
        return molecular
    ...
    @property
    def average_roughness(self, singular_list: bool = False) -> Union[float, List[float]]:
        if len(self._average_roughnesses) > 1 or singular_list:
            return self._average_roughnesses.copy()
        return self._average_roughnesses[0]

    @property
    def attrv_adj(self, singular_list: bool = False) -> Union[float, List[float]]:
        attrv_adj_list = [params["attrv_adj"] for params in self._roughness_calc_params]
        if len(attrv_adj_list) > 1 or singular_list:
            return attrv_adj_list
        return attrv_adj_list[0]

    @property
    def precision(self, singular_list: bool = False) -> Union[int, List[int]]:
        precision_list = [params["precision"] for params in self._roughness_calc_params]
        if len(precision_list) > 1 or singular_list:
            return precision_list
        return precision_list[0]

    @property
    def scan_step(self, singular_list: bool = False) -> Union[float, List[float]]:
        scan_step_list = [params["scan_step"] for params in self._roughness_calc_params]
        if len(scan_step_list) > 1 or singular_list:
            return scan_step_list
        return scan_step_list[0]

    @property
    def delta_z(self, singular_list: bool = False) -> Union[float, List[float]]:
        delta_z_list = [params["delta_z"] for params in self._roughness_calc_params]
        if len(delta_z_list) > 1 or singular_list:
            return delta_z_list
        return delta_z_list[0]

    @property
    def roughness_calc_params(self, singular_list: bool = False) -> Union[Dict, List[Dict]]:
        if len(self._roughness_calc_params) > 1 or singular_list:
            return deepcopy(self._roughness_calc_params)
        return self._roughness_calc_params[0].copy()

    @property
    def is_smoothest(self, singular_list: bool = False) -> Union[bool, List[bool]]:
        if len(self._smoothest) > 1 or singular_list:
            return self._smoothest.copy()
        return self._smoothest[0]
...
class Terminator:
    def __init__(
        ...
        self._shifted_dummy_cells = self._apply_shifts()
        ...
        self._terminations = (
            terminations if terminations else [None] * len(self._shifted_dummy_cells)
        )
    ...
    def rotation_params(self) -> Dict:
        return self._rotation_params.copy()
    ...
    def average_roughnesses(self) -> List[float]:
        ...
        return self._average_roughnesses

    @property
    def terminated_structures(self, unrotate: bool = True, dummy: bool = False) -> List[Structure]:
        cells = self._shifted_dummy_cells if dummy else self._shifted_cells
    ...
    @property
    def terminations(self, force_roughness_calc: bool = False) -> List[Termination]:
    ...
    def _apply_shifts(self) -> List[Structure]:
        shifted_obses = []
    ...
    def _undummify(self) -> List[Structure]:
        undummified = []
        for dummy in self._shifted_dummy_cells:
        ...
        for i, termination, roughness in enumerate(zip(self._terminations, average_roughnesses)):
            ...
            if termination is None:
                self._terminations[i] = Termination(
                    rotated_dummy_structure=self._shifted_dummy_cells[i],
    ...
    def output_files(self, typ: str = "smoothest", unrotate: bool = True, dummy: bool = False):
        if typ == "all":
...
def homogenize(points: np.ndarray) -> np.ndarray:
    ...
    sys.stdout.write(
        f'\nA "points" array with a weird shape was passed to homogenize()! points.shape = {points.shape}\n\n'
    )
    return points
